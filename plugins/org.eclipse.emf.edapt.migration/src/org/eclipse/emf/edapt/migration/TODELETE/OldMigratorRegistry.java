 ...
 * @levd.rating RED Rev:
 */
public class OldMigratorRegistry {

	/** Singleton instance. */
	private static OldMigratorRegistry migratorRegistry;

	/** Registered migrators identified by unversioned namespace URI. */
	private final Map<String, OldMigrator> migrators;
	...
	private OldMigratorRegistry() {
		migrators = new HashMap<String, OldMigrator>();
	...
	/** Getter for instance. */
	public static OldMigratorRegistry getInstance() {
		if (migratorRegistry == null) {
			migratorRegistry = new OldMigratorRegistry();
		}
	...
	public void registerMigrator(URL migratorURL) throws MigrationException {
		OldMigrator migrator = new OldMigrator(URIUtils.getURI(migratorURL));
	...
	public OldMigrator getMigrator(String nsURI) {
		return migrators.get(nsURI);
	}

	/** Get a migrator for a certain model. */
	public OldMigrator getMigrator(URI modelURI) {
